...
export const getProducts = async (req, res, next) => {
  try {
    ...
    const { sort, category, minPrice, maxPrice, brand, minRating, inStock } =
      req.query;
...
export const createProduct = async (req, res, next) => {
  ...
  if (req.file) {
    if (!cloudinaryConfigured()) {
      return next(
        new AppError(
          "File uploads are not configured. Please use an image URL instead.",
          503
        )
      );
    }
...
export const getRelatedProducts = async (req, res) => {
  ...
  try {
    ...
    if (targetTagsSet.size > 0)
      orConditions.push({ tags: { $in: [...targetTagsSet] } });
...
export const getProductBundle = async (req, res) => {
  ...
  try {
    ...
    const bundleTotal = [product, ...items.map((i) => i.product)].reduce(
      (sum, p) => sum + p.price,
      0
    );
  ...
  try {
    ...
    if (hasQuery && brandList.length === 0) {
      ...
      if (esProducts) {
        return res
          .status(200)
          .json({ success: true, count: esProducts.length, data: esProducts });
      }
    }
    ...
    if (brandList.length > 0) {
      filter.brand = {
        $in: brandList.map((b) => new RegExp(`^${escapeRegex(b)}$`, "i")),
      };
    }
